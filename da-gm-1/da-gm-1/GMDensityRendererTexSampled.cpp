	...
	for (int i = 0; i < SIZE; ++i) {
		for (int j = 0; j < SIZE; ++j) {
			for (int k = 0; k < SIZE; ++k) {
				...
				float val = mixture->sample(double(k) / 2.0f - SIZE/4.0f,
					double(j) / 2.0f - SIZE/4.0f,
					double(i) / 2.0f - SIZE/4.0f);
				//float val = 0.03 - std::sqrt(std::pow(k-10,2) + std::pow(j-10,2) + std::pow(i-10,2)) / 1000.0;
				samples[i * SIZE*SIZE + j * SIZE + k] = val;
				if (val > max) max = val;
			}
		}
	}

	m_gl->glTexImage3D(GL_TEXTURE_3D, 0, GL_R16, SIZE, SIZE, SIZE, 0, GL_RED, GL_FLOAT, samples);
}
