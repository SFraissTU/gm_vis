...
GMDensityRendererDirectSampled::GMDensityRendererDirectSampled(QOpenGLFunctions_4_5_Core* gl, DisplaySettings* settings, Camera* camera, int width, int height) : m_gl(gl), m_settings(settings), m_camera(camera) {
	m_program = std::make_unique<QOpenGLShaderProgram>();
	m_program->addShaderFromSourceFile(QOpenGLShader::Compute, "shaders/density_notex.comp");
	m_program->link();
...
void GMDensityRendererDirectSampled::setMixture(GaussianMixture* mixture)
{
	m_mixture = mixture;
	size_t arrsize;
...
void GMDensityRendererDirectSampled::setSize(int width, int height)
{
...
void GMDensityRendererDirectSampled::render()
{
	if (!m_mixture) {
		return;
	}
...
void GMDensityRendererDirectSampled::cleanup()
{
